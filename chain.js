...
const whichCourses = {
  text: `לתחום היל"ה בבסמ"ח יש מגוון רחב של קורסים בתחום הסייבר, תכנות, תשתיות ומחשוב אישי. \n 
  על מנת להיות מעודכנים בכלל הקורסים שלנו, יש לחפש בצה"לנט 'מרש"ל', או להצטרף לקבוצת טלגרם "השתלמויות בסמ"ח" שם אנחנו מפרסמים בכל תחילת חודש את הקורסים שנפתחים, לינק לקבוצה:  \n
  https://t.me/joinchat/vc9o_7Ved5E5YWRk \n
  במרש"ל תוכלו להתעדכן במגוון הקורסים, תאריכים ומידע נוסף.`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const gmush = {
  text: `שעות הזכאות לגמו"ש משתנות בין הקורסים, וישנם קורסים שלא מוכרים לגמו"ש כלל. באתר המרש"ל ברשת הצהלנ"ט תוכלו לקבל את כל המידע אודות הקורס שמעניין אתכם.\n
  יש לציין כי הגמו"ש הצבאי יוענק ע"י מופ"ת בלבד, ובאחריותם בלבד! לכל שאלה אחרת בנושא גמו"ש יש לפנות למופ"ת.`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const register = {
  text: `על מנת להירשם לקורס יש לפנות אל רשם הקורסים של היחידה שלכם. איך יודעים מי רשם הקורסים שלי? יש לפנות אל שלישות היחידה שלכם שבדר"כ ידעו לתת לכם מענה בנושא`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
  ...
  opts: {
    ...
    reply_markup: JSON.stringify({
      inline_keyboard: [
        [
          {
            text: "מידע כללי (מיקום, בין אלו שעות מתקיים וכו')",
...
const frontalGeneralInformation = {
  text: `כשבוע לפני תחילת הקורס תישלח למייל הצה"לי של כל משתלם הוראת זימון מפורטת עם כל הפרטים אודות מיקום, תאריכים, שעות, סימול וכו'. בנוסף, תיפתח קבוצת ווטסאפ למשתלמים.\n
  צווי סיפוח – יש לשלוח צווי סיפוח במייל הצה"לי ל 6020 + Cntrl k עם הסימול אשר מופיע בהוראת הזימון או להגיע עם צו הסיפוח מודפס ביום הראשון לקורס.
  `,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
...
const testGradeInformation = {
  text: `ציון מבחן יוזן ב'קיוסק אישי' בצה"לנט עד שבועיים לאחר מועד הבחינה (ייתכנו חריגות במקרים מיוחדים) .`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const objectTestGradeInformation = {
  text: `על מנת לערער על ציון יש לשלוח מייל בצה"לנט ל 6020 + Ctrl k`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const gettingDiplomaInformation = {
  ...
  במידה ואתם אכן זכאים לתעודה, היא תשלח אליכם במייל הצה"לי
  `,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
...
const cancelRegistration = {
  text: `ביטול רישום לקורס יתבצע מול המוקד הטלפוני שלנו. יש לציין כי ביטול משתלם יתבצע עד שבועיים לפני תחילת הקורס. אם הביטול יתבצע פחות משבועיים לפני תחילת הקורס - היחידה לא תזוכה תקציבית. `,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const hybridGeneralInformation = {
  text: `הקורס יתבצע בפלטפורמה מקוונת אונליין - בדר"כ באמצעות זום. כשבוע לפני תחילת הקורס תפתח קבוצת ווטסאפ עם הפרטים המלאים.`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const onlineGeneralInformation = {
  text: `הקורס יתבצע בלמידה עצמאית מרחוק ע"י מערכת ה'קנבס' בצה"לנט. ביום הפתיחה של הקורס ישלח אליכם למייל הצה"לי פרטים על מועד המבחן, מיקום והשעה בה תצטרכו להגיע`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const whenEnterTest = {
  text: `חלק מהקורסים שיש לנו להציע מחייבים מבחן כניסה לפני קליטה לקורס עצמו. על מנת לדעת אילו קורסים דורשים מבחן כניסה יש לחפש בצה"לנט 'מרש"ל', ובכל קורס אשר תבחרו באתר יהיה כתוב בפרטיו האם דרוש מבחן כניסה או לא.`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const checkRegitserEnterTest = {
  text: `תוכלו לוודא זאת מול רשם הקורסים היחידתי או לשלוח אלינו מייל בצה"לנט – 6020 + Ctrl k`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const cencelEnterTest = {
  text: `על מנת לבטל/לדחות רישום למבחן כניסה יש לדבר עם רשם הקורסים של היחידה שלכם או לשלוח אלינו מייל בצה"לנט– 6020 + Ctrl k`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
  ...
  opts: {
    ...
    reply_markup: JSON.stringify({
      inline_keyboard: [
        [
          {
            text: "איך ניתן לדית אילו קורסים מהווים שלב עבור המקצוע שלי?",
        ...
        [
          {
            text: `מידע על תעודה לצורך שלב בסקו"פ`,
...
const allowedLevelTests = {
  text: `יש לשלוח אלינו מייל בצה"לנט - ctrl K + 6020 עם הפרטים הבאים: מספר אישי, מקצוע החייל`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const noLevelTests = {
  text: `יש לשלוח לנו מייל בצה"לנט – 6020 + Ctrl k עם הפרטים הבאים: מספר אישי, מקצוע החייל, שלב להזנה ושם הקורס שבוצע`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const registerLevelTestsScoop = {
  ...
  3.	בוחרים בתיבה המתאימה לפי סוג המקצוע. הרישום יתבצע ע"י רשם הקורסים היחידתי.
...
const whatIsScoop = {
  text: `סקו"פ – סביבת קורסים רשתית פתוחה, הינו פרוייקט המאפשר לרכוש קורסים מקצועיים באתרים אזרחיים כגון udemy.  `,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const registerScoop = {
  ...
  1)	באתר ה'מרש"ל' – מחפשים בשורת החיפוש 'מדרגה' (מדרגת מחיר). הרישום יתבצע ע"י רשם הקורסים היחידתי לפי מדרגת המחיר המתאימה.
       ...
       במידה ולא קיים מחזור פתוח לרישום עבור המדרגה הרצויה, יש לפנות אלינו במייל בצה"לנט – 6020 + Ctrl k .
...
const whatIsCanvas = {
  text: `קנבס היא מערכת א-סינכרוני בצה"לנט המשמשת להעברת השתלמויות רשתיות. למידה רשתית בקנבס כוללת סרטונים נוחים למשתמש, תרגולים וסיכומי פרקים של חלק מן הקורסים. הקורס פתוח לפרק זמן של 3 שבועות והיקף הקורסים 40 שעות. נכון להיום הקורסים מזוכים לגמו"ש.`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
...
const diplomaCourse = {
  ...
  *לבקשת תעודה יש לשלוח לנו מייל בצה"לנט – 6020 + Ctrl k עם הפרטים הבאים: שם מלא, מספר אישי, דרגה והקורס אשר ביצעתם.`,
  opts: {
    parse_mode: "HTML",
    reply_markup: JSON.stringify({
      inline_keyboard: [
